...
public class WordsGrid : MonoBehaviour
{
        ...
        if(currentGameData != null)
        {
            ...
            foreach (var squares in currentGameData.selectedBoardData.Board)
            {
                foreach (var squareLetter in squares.Row)
                {
                    ...
                    {
                        Debug.LogError(
                            "All field in your array should have some letter. Press Fill up with random button in your board data to add random letter. Letter: " + squareLetter
                        );
    ...
    private bool ShouldScaleDown(Vector3 targetScale)
    {
        ...
        var midWidthPosition = ((currentGameData.selectedBoardData.Columns * squareSize.x)/ 2) * 0.1f;
        var midWidthHeight = ((currentGameData.selectedBoardData.Columns * squareSize.y)/ 2) * 0.1f;
